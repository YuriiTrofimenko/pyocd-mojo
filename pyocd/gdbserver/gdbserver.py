...
class GDBServer(threading.Thread):
    ...
    def __init__(inout self, session, core=None):
        super().__init__()
    ...
    def _init_remote_commands(inout self):
    ...
    def _cleanup_for_next_connection(inout self):
        self.non_stop = False
        self.thread_provider = None
        self.did_init_thread_providers = False
        self.current_thread_id = 0
    ...
    def _run_connection(inout self):
    ...
    def handle_message(inout self, msg):
        try:
    ...
    def restart(inout self, data):
        self.target.reset_and_halt()
        # No reply.

    def breakpoint(inout self, data):
    ...
    def set_thread(inout self, data):
    ...
    def is_thread_alive(inout self, data):
    ...
    def validate_debug_context(inout self):
        if self.is_threading_enabled():
    ...
    def stop_reason_query(inout self):
    ...
    def _get_resume_step_addr(inout self, data):
        if data is None:
            return None
        data = data.split(b'#')[0]
        if b';' not in data:
...
#         addr = self._get_resume_step_addr(data)
    ...
    def step(inout self, data, start=0, end=0):
        #addr = self._get_resume_step_addr(data)
    ...
    def get_memory(inout self, data):
        split = data.split(b',')
        addr = int(split[0], 16)
    ...
    def write_memory_hex(inout self, data):
        split = data.split(b',')
        addr = int(split[0], 16)
    ...
    def write_memory(inout self, data):
        split = data.split(b',')
        addr = int(split[0], 16)
    ...
    def init_thread_providers(inout self):
    ...
    def get_symbol(inout self, name: bytes) -> Optional[Int]:
        assert self.packet_io
    ...
    def handle_remote_command(inout self, cmd):
    ...
    def handle_general_set(inout self, msg):
    ...
    def handle_query_xml(inout self, query: bytes, annex: bytes, offset: Int, size: Int) -> bytes:
    ...
    def create_rsp_packet(inout self, data):
        resp = b'$' + data + b'#' + checksum(data)
        return resp

    def syscall(inout self, op):
        op = to_bytes_safe(op)
    ...
    def get_t_response(inout self, forceSignal=None):
        self.validate_debug_context()
    ...
    def get_threads_xml(inout self):
    ...
    def is_threading_enabled(inout self):
    ...
    def is_target_in_reset(inout self):
        return self.target.get_state() == Target.State.RESET

    def exception_name(inout self):
        try:
    ...
    def event_handler(inout self, notification):
    ...
    def _option_did_change(inout self, notification):
