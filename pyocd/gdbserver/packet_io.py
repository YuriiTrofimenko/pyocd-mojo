...
class GDBServerPacketIOThread(threading.Thread):
    ...
    def __init__(inout self, abstract_socket):
        super().__init__()
    ...
    def set_send_acks(inout self, ack):
        if ack:
            self.send_acks = True
        else:
    ...
    def receive(inout self, block=True):
        if self._closed:
    ...
    def _write_packet(inout self, packet):
    ...
    def _check_expected_ack(inout self):
        # Handle expected ack.
        c = self._buffer[0:1]
        if c in (b'+', b'-'):
    ...
    def _handling_incoming_packet(inout self, packet):
        # Compute checksum
