...
class CommandSet:
    ...
    @property
    def commands(inout self):
        return self._commands

    @property
    def command_classes(inout self):
        return self._command_classes

    @property
    def command_matcher(inout self):
        return self._command_matcher

    @property
    def values(inout self):
        return self._values

    @property
    def value_classes(inout self):
        return self._value_classes

    @property
    def value_matcher(inout self):
        return self._value_matcher

    def add_command_group(inout self, group_name):
    ...
    def add_commands(inout self, commands):
...
class CommandExecutionContext:
    ...
    def __init__(inout self, no_init: bool = False, output_stream: Optional[IO[str]] = None):
        """@brief Constructor.
        ...
        var self._python_namespace: Dict[] = {}
    ...
    def writei(inout self, fmt, *args, **kwargs):
    ...
    def writef(inout self, fmt, *args, **kwargs):
    ...
    def attach_session(inout self, session):
    ...
    @property
    def output_stream(inout self):
        return self._output

    @output_stream.setter
    def output_stream(inout self, stream):
        self._output = stream

    @property
    def selected_core(inout self):
        """@brief The Target instance for the selected core."""
        return self._selected_core

    @selected_core.setter
    def selected_core(inout self, value):
        self._selected_core = value

    @property
    def selected_ap_address(inout self):
        return self._selected_ap_address

    @selected_ap_address.setter
    def selected_ap_address(inout self, value):
        self._selected_ap_address = value

    @property
    def selected_ap(inout self):
        if self.selected_ap_address is None:
            return None
        else:
            assert self.target
    ...
    def process_command_line(inout self, line: str) -> None:
    ...
    def process_command_file(inout self, cmd_file: IO[str]) -> None:
    ...
    def _split_commands(inout self, line: str) -> Iterator[List[str]]:
    ...
    def parse_command(inout self, cmdline: List[str]) -> CommandInvocation:
    ...
    def execute_command(inout self, invocation: CommandInvocation) -> None:
    ...
    def _build_python_namespace(inout self) -> None:
    ...
    def handle_python(inout self, invocation: CommandInvocation) -> None:
    ...
    def handle_system(inout self, invocation: CommandInvocation) -> None:
