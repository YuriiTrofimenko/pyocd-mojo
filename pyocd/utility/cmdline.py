...
def split_command_line(cmd_line: Union[]) -> List[str]:
...
def convert_vector_catch(vcvalue: Union[]) -> Int:
    ...
    var value: str = to_str_safe(vcvalue).lower()
...
def convert_one_session_option(name: str, value: Optional[str]) -> Tuple[]:
...
def convert_session_options(option_list: Iterable[str]) -> Dict[]:
...
def convert_reset_type(value: str) -> Optional[]:
