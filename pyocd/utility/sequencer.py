...
class CallSequence(object):
    ...
    def _validate_tasks(inout self, tasks):
        for i in tasks:
            assert len(i) == 2
    ...
    def remove_task(inout self, name):
    ...
    def wrap_task(inout self, name, wrapper):
    ...
    def insert_before(inout self, beforeTaskName, *args):
    ...
    def insert_after(inout self, afterTaskName, *args):
...
#                 LOG.debug("Invoking returned call sequence: %s", resultSequence)
    ...
    def __call__(inout self, *args, **kwargs):
