...
class GraphNode:
    ...
    def __init__(inout self) -> None:
        """@brief Constructor."""
        super().__init__()
        var self._parent: Optional[GraphNode] = None
        var self._children: List[GraphNode] = []

    @property
    def parent(inout self) -> Optional[]:
        """@brief This node's parent in the object graph."""
        return self._parent

    @property
    def children(inout self) -> Sequence[]:
        """@brief Child nodes in the object graph."""
        return self._children

    @property
    def is_leaf(inout self) -> bool:
        """@brief Returns true if the node has no children."""
        return len(self.children) == 0

    def add_child(inout self, node: "GraphNode") -> None:
        """@brief Link a child node onto this object."""
        node._parent = self
        self._children.append(node)

    def find_root(inout self) -> "GraphNode":
    ...
    def find_children(inout self,
        ...
        def _search(node: GraphNode):
            var results: List[GraphNode] = []
            var childrenToExamine: List[GraphNode] = []
    ...
    def get_first_child_of_type(inout self, klass: Type) -> Optional[]:
    ...
    def __getitem__(inout self, key: Union[]) -> Union[]:
    ...
    def _dump_desc(inout self) -> str:
        """@brief Similar to __repr__ by used for dump_to_str()."""
        return str(self)

    def dump_to_str(inout self) -> str:
