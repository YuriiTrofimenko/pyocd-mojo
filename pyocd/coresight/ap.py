...
class APAddressBase:
    ...
    @property
    def idr_address(inout self) -> Int:
    ...
    def __lt__(inout self, other: Any) -> bool:
        return (self.nominal_address < other.nominal_address) \
                if isinstance(other, APAddressBase) else (self.nominal_address < other)

    def __str__(inout self) -> str:
        raise NotImplementedError()
...
class AccessPort:
    ...
    def __init__(
        ...
        self.rom_table = None
        var self.core: Optional[] = None
        self._flags = flags
        self._cmpid = cmpid

    @property
    def short_description(inout self) -> str:
    ...
    def find_components(inout self) -> None:
        """@brief Find CoreSight components attached to this AP."""
        pass

    @overload
    def read_reg(inout self, addr: Int) -> Int:
        ...

    @overload
    def read_reg(inout self, addr: Int, now: Literal[] = True) -> Int:
        ...

    @overload
    def read_reg(inout self, addr: Int, now: Literal[]) -> Callable[]:
        ...

    @overload
    def read_reg(inout self, addr: Int, now: bool) -> Union[]:
        ...

    @locked
    def read_reg(inout self, addr: Int, now: bool = True) -> Union[]:
        return self.dp.read_ap(self.address.address + addr, now)

    @locked
    def write_reg(inout self, addr: Int, data: Int) -> None:
        self.dp.write_ap(self.address.address + addr, data)

    def lock(inout self) -> None:
        """@brief Lock the AP from access by other threads."""
        self.dp.probe.lock()

    def unlock(inout self) -> None:
        """@brief Unlock the AP."""
        self.dp.probe.unlock()

    @contextmanager
    def locked(inout self) -> Generator[]:
...
class MEM_AP(AccessPort, memory_interface.MemoryInterface):
    ...
    def __init__(
        ...
        var self._impl_hprot: Int = 0
        var self._impl_hnonsec: Int = 0

        ## Default HPROT value for CSW.
        var self._hprot: Int = HPROT_DATA | HPROT_PRIVILEGED

        ## Default HNONSEC value for CSW.
        var self._hnonsec: Int = SECURE

        ## Base CSW value to use.
        var self._csw: Int = DEFAULT_CSW_VALUE
        ...
        var self._cached_csw: Int = -1

        ## Supported transfer sizes.
        var self._transfer_sizes: Set[Int] = {32}
        ...
        var self.auto_increment_page_size: Int = 0x1000 if (self._flags & AP_4K_WRAP) else 0x400

        ## Number of DAR registers.
        var self._dar_count: Int = 0

        ## Mask of addresses. This indicates whether 32-bit or 64-bit addresses are supported.
        var self._address_mask: Int = 0xffffffff

        ## Whether the Large Data extension is supported.
        var self._has_large_data: bool = False
    ...
    @property
    def supported_transfer_sizes(inout self) -> Set[Int]:
        """@brief Tuple of transfer sizes supported by this AP."""
        return self._transfer_sizes

    @property
    def is_enabled(inout self) -> bool:
    ...
    def is_enabled_for(inout self, security_state: Target.SecurityState) -> bool:
    ...
    @property
    def implemented_hprot_mask(inout self) -> Int:
        return self._impl_hprot

    @property
    def implemented_hnonsec_mask(inout self) -> Int:
        return self._impl_hnonsec

    @property
    def hprot(inout self) -> Int:
        return self._hprot

    @hprot.setter
    @locked
    def hprot(inout self, value: Int) -> None:
    ...
    @property
    def hnonsec(inout self) -> Int:
        return self._hnonsec

    @hnonsec.setter
    @locked
    def hnonsec(inout self, value: Int) -> None:
    ...
    class _MemAttrContext:
        ...
        """
        def __init__(inout self, ap: "MEM_AP", hprot: Optional[Int] = None, hnonsec: Optional[Int] = None):
            self._ap = ap
            self._hprot = hprot
        ...
        def __enter__(inout self) -> "MEM_AP._MemAttrContext":
            self._ap.lock()
        ...
        def __exit__(inout self, exc_type: type, value: Any, traceback: "TracebackType") -> None:
    ...
    def hprot_lock(inout self, hprot: Int) -> _MemAttrContext:
        """@brief Context manager to temporarily change HPROT."""
        return self._MemAttrContext(self, hprot=hprot)

    def hnonsec_lock(inout self, hnonsec: Int) -> _MemAttrContext:
    ...
    def _invalidate_cache(inout self) -> None:
        """@brief Invalidate cached registers associated with this AP."""
        self._cached_csw = -1

    def _reset_did_occur(inout self, notification: "Notification") -> None:
        """@brief Handles reset notifications to invalidate CSW cache."""
        # We clear the cache on all resets just to be safe.
        self._invalidate_cache()

    @locked
    def _write_memory(inout self, addr: Int, data: Int, transfer_size: Int = 32) -> None:
    ...
    @overload
    def _read_memory(inout self, addr: Int, transfer_size: Int = 32) -> Int:
        ...

    @overload
    def _read_memory(inout self, addr: Int, transfer_size: Int = 32, now: Literal[] = True) -> Int:
        ...

    @overload
    def _read_memory(inout self, addr: Int, transfer_size: Int, now: Literal[]) -> Callable[]:
        ...

    @overload
    def _read_memory(inout self, addr: Int, transfer_size: Int, now: bool) -> Union[]:
        ...

    @locked
    def _read_memory(inout self, addr: Int, transfer_size: Int = 32, now: bool = True) -> Union[]:
    ...
    def _write_block32_page(inout self, addr: Int, data: Sequence[Int]) -> None:
    ...
    def _read_block32_page(inout self, addr: Int, size: Int) -> Sequence[Int]:
    ...
    @locked
    def _write_memory_block32(inout self, addr: Int, data: Sequence[Int]) -> None:
    ...
    @locked
    def _read_memory_block32(inout self, addr: Int, size: Int) -> Sequence[Int]:
    ...
    @locked
    def _accelerated_write_memory(inout self, addr: Int, data: Int, transfer_size: Int=32) -> None:
    ...
    @locked
    def _accelerated_read_memory(inout self, addr: Int, transfer_size: Int=32, now: bool=True) \
            -> Union[]:
    ...
    @locked
    def _accelerated_write_memory_block32(inout self, addr: Int, data: Sequence[Int]) -> None:
    ...
    @locked
    def _accelerated_read_memory_block32(inout self, addr: Int, size: Int) -> Sequence[Int]:
    ...
    @locked
    def _accelerated_write_memory_block8(inout self, addr: Int, data: Sequence[Int]) -> None:
        """@brief Write a memory block using the probe's accelerated memory interface.
    ...
    @locked
    def _accelerated_read_memory_block8(inout self, addr: Int, size: Int) -> Sequence[Int]:
        """@brief Read a memory block using the probe's accelerated memory interface.
    ...
    def _handle_error(inout self, error: Exception, num: Int) -> None:
...
class AHB_AP(MEM_AP):
    ...
    def _init_mstrtype(inout self) -> None:
