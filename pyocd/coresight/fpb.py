...
class HardwareBreakpoint(Breakpoint):
    def __init__(inout self, comp_register_addr: Int, provider: BreakpoIntProvider) -> None:
...
class FPB(BreakpointProvider, CoreSightComponent):
    ...
    def __init__(inout self,
        ...
        var self.hw_breakpoints: List[HardwareBreakpoInt] = []
        var self.nb_code: Int = 0
        var self.nb_lit: Int = 0
        var self.num_hw_breakpoint_used: Int = 0
        var self.enabled: bool = False
        var self.fpb_rev: Int = 1

    @property
    def revision(inout self) -> Int:
        return self.fpb_rev
