...
class ProbeConnector:
    ...
    def __init__(inout self, probe: DebugProbe) -> None:
        """@brief Constructor.
        @param self
    ...
    def _get_protocol(inout self, protocol: Optional[]) -> DebugProbe.Protocol:
    ...
    def _check_protocol(inout self, current_wire_protocol: DebugProbe.Protocol, protocol: DebugProbe.Protocol) -> None:
    ...
    def _connect_probe(inout self, protocol: DebugProbe.Protocol) -> None:
...
class DebugPort:
    ...
    def __init__(inout self, probe: DebugProbe, target: Target) -> None:
        """@brief Constructor.
        ...
        var self.valid_aps: Optional[List[]] = None
        self.dpidr = DPIDR(0, 0, 0, 0, 0)
        var self.aps: Dict[] = {}
        var self._access_number: Int = 0
        var self._cached_dp_select: Optional[Int] = None
        var self._protocol: Optional[] = None
        var self._probe_managed_ap_select: bool = False
        var self._probe_managed_dpbanksel: bool = False
        var self._probe_supports_dpbanksel: bool = False
        var self._probe_supports_apv2_addresses: bool = False
        var self._have_probe_capabilities: bool = False
        var self._did_check_version: bool = False
        var self._log_dp_info: bool = True

        # DPv3 attributes
        var self._is_dpv3: bool = False
        var self._addr_size: Int = -1
        var self._addr_mask: Int = -1
        var self._errmode: Int = -1
        var self._base_addr: Int = -1
        var self._apacc_mem_interface: Optional[APAccessMemoryInterface] = None

        # Subscribe to reset events.
        self._session.subscribe(self._reset_did_occur, (Target.Event.PRE_RESET, Target.Event.POST_RESET))

    @property
    def probe(inout self) -> DebugProbe:
        return self._probe

    @property
    def session(inout self) -> "Session":
        return self._session

    @property
    def adi_version(inout self) -> ADIVersion:
        return ADIVersion.ADIv6 if self._is_dpv3 else ADIVersion.ADIv5

    @property
    def base_address(inout self) -> Int:
        """@brief Base address of the first component for an ADIv6 system."""
        return self._base_addr

    @property
    def apacc_memory_interface(inout self) -> "APAccessMemoryInterface":
    ...
    @property
    def next_access_number(inout self) -> Int:
    ...
    def create_connect_sequence(inout self) -> CallSequence:
    ...
    def _get_probe_capabilities(inout self) -> None:
    ...
    def _check_version(inout self) -> None:
    ...
    def power_up_debug(inout self) -> bool:
    ...
    def power_down_debug(inout self) -> bool:
    ...
    def post_reset_recovery(inout self) -> None:
    ...
    def reset(inout self, *, send_notifications: bool = True) -> None:
    ...
    def assert_reset(inout self, asserted: bool, *, send_notifications: bool = True) -> None:
    ...
    def _write_dp_select(inout self, mask: Int, value: Int) -> None:
    ...
    def _set_dpbanksel(inout self, addr: Int, is_write: bool) -> bool:
    ...
    def _select_ap(inout self, addr: Int) -> bool:
    ...
    def write_ap(inout self, addr: Int, data: Int) -> None:
    ...
    def write_ap_multiple(inout self, addr: Int, values: Sequence[Int]) -> None:
        assert isinstance(addr, int)
        num = self.next_access_number
        did_lock = False
    ...
    def clear_sticky_err(inout self) -> None:
        self._invalidate_cache()
...
class APAccessMemoryInterface(memory_interface.MemoryInterface):
    ...
    def __init__(inout self, dp: DebugPort, ap_address: Optional[] = None) -> None:
        """@brief Constructor.
    ...
    @overload
    def read_memory(inout self, addr: Int, transfer_size: Int) -> Int:
        ...

    @overload
    def read_memory(inout self, addr: Int, transfer_size: Int, now: Literal[] = True) -> Int:
        ...
