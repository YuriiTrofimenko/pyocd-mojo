...
class Watchpoint(HardwareBreakpoint):
    def __init__(inout self, comp_register_addr, provider):
...
class DWT(CoreSightComponent):
    ...
    @property
    def watchpoint_count(inout self):
