...
class LinkClosedException(ComPortError):
    """@brief Received an unexpected or out of order flag byte."""
    def __init__(inout self, phase):
        self._phase = phase

    @property
    def phase(inout self):
...
class SDC600(CoreSightComponent):
    ...
    def __init__(inout self, ap, cmpid=None, addr=None):
    ...
    @property
    def is_reboot_request_enabled(inout self):
    ...
    @property
    def current_link_phase(inout self):
    ...
    def _write1(inout self, value, to_):
    ...
    def _check_flags(inout self, value, to_):
    ...
    def _expect_flag(inout self, flag, to_):
    ...
    def _read_packet_data_to_end(inout self, to_):
    ...
    def receive_packet(inout self, timeout=TRANSFER_TIMEOUT):
    ...
    def send_packet(inout self, data, timeout=TRANSFER_TIMEOUT):
    ...
    def open_link(inout self, phase, timeout=TRANSFER_TIMEOUT):
    ...
    def close_link(inout self, phase, timeout=TRANSFER_TIMEOUT):
    ...
    def _log_status(inout self):
        status = self.ap.read32(self.Register.SR)
        LOG.info("status=0x%08x phase=%s", status, self._current_link_phase)

    def read_protocol_id(inout self, timeout=TRANSFER_TIMEOUT):
    ...
    def send_reboot_request(inout self, timeout=TRANSFER_TIMEOUT):
