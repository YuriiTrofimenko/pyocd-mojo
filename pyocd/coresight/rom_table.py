...
class CoreSightComponentID(object):
    ...
    def __init__(inout self, parent_rom_table, ap, top_addr, power_id=None):
    ...
    def read_id_registers(inout self):
    ...
    def _extract_id_register_value(inout self, regs, offset):
        result = 0
        for i in range(4):
            value = regs[offset + i]
    ...
    @property
    def designer_desc(inout self) -> str:
...
class ROMTable(CoreSightComponent):
    ...
    def __init__(inout self, ap, cmpid=None, addr=None, parent_table=None):
        """@brief Constructor."""
        assert cmpid is not None
    ...
    @property
    def depth(inout self):
        """@brief Number of parent ROM tables."""
        return self._depth

    @property
    def components(inout self):
    ...
    def _read_table(inout self):
        raise NotImplementedError()

    def for_each(inout self, action, filter=None):
...
class Class1ROMTable(ROMTable):
    ...
    def _power_component(inout self, number, powerid, entry):
        if self.gpr is None:
    ...
    def _handle_table_entry(inout self, entry, number):
...
class Class9ROMTable(ROMTable):
    ...
    def check_power_request_version(inout self):
    ...
    def power_debug_domain(inout self, domain_id, enable=True):
