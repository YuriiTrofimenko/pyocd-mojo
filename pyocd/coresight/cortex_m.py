...
class CortexM(CoreTarget, CoreSightCoreComponent): # lgtm[py/multiple-calls-to-init]
    ...
    def __init__(inout self,
        ...
        var self._architecture: CoreArchitecture = CoreArchitecture.ARMv6M
        var self._extensions: List[CortexMExtension] = []
        self.core_type = 0
        var self.has_fpu: bool = False
        var self._core_number: Int = core_num
        var self._core_name: str = "Unknown"
        var self._run_token: Int = 0
        var self._target_context: Optional[] = None
        self._elf = None
        self.target_xml = None
        self._core_registers = CoreRegistersIndex()
        var self._supports_vectreset: bool = False
        var self._reset_catch_delegate_result: DelegateResult = False
        var self._reset_catch_saved_demcr: Int = 0
        var self.fpb: Optional[FPB] = None
        var self.dwt: Optional[DWT] = None
    ...
    def add_child(inout self, cmp: "CoreSightComponent") -> None:
    ...
    @property
    def core_number(inout self) -> Int:
        return self._core_number

    @property
    def architecture(inout self) -> CoreArchitecture:
        """@brief @ref pyocd.coresight.core_ids.CoreArchitecture "CoreArchitecture" for this core."""
        return self._architecture

    @property
    def extensions(inout self) -> List[CortexMExtension]:
    ...
    @property
    def elf(inout self) -> Optional[]:
        return self._elf

    @elf.setter
    def elf(inout self, elffile: "ELFBinaryFile") -> None:
        self._elf = elffile

    @property
    def default_reset_type(inout self) -> Target.ResetType:
        return self._default_reset_type

    @default_reset_type.setter
    def default_reset_type(inout self, reset_type: Target.ResetType) -> None:
        assert isinstance(reset_type, Target.ResetType)
        self._default_reset_type = reset_type

    @property
    def default_software_reset_type(inout self) -> Target.ResetType:
        return self._default_software_reset_type

    @default_software_reset_type.setter
    def default_software_reset_type(inout self, reset_type: Target.ResetType) -> None:
    ...
    def _build_registers(inout self) -> None:
    ...
    def _read_core_type(inout self) -> None:
    ...
    def _check_for_fpu(inout self) -> None:
    ...
    def clear_debug_cause_bits(inout self):
    ...
    def _perform_emulated_reset(inout self):
    ...
    def _get_actual_reset_type(inout self, reset_type):
    ...
    def _perform_reset(inout self, reset_type):
    ...
    def _post_reset_core_accessibility_test(inout self):
    ...
    def set_reset_catch(inout self, reset_type):
    ...
    def clear_reset_catch(inout self, reset_type):
    ...
    def check_reg_list(inout self, reg_list):
    ...
    def _base_read_core_registers_raw(inout self, reg_list):
    ...
    def _base_write_core_registers_raw(inout self, reg_list, data_list):
    ...
    @property
    def available_breakpoint_count(inout self):
        return self.fpb.available_breakpoints if self.fpb else 0

    def find_watchpoint(inout self, addr, size, type):
        if self.dwt is not None:
    ...
    def remove_watchpoint(inout self, addr, size=None, type=None):
    ...
    def set_vector_catch(inout self, enableMask):
        demcr = self.read_memory(CortexM.DEMCR)
    ...
    def exception_number_to_name(inout self, exc_num: Int, name_thread: bool = False) -> Optional[str]:
    ...
    def in_thread_mode_on_main_stack(inout self) -> bool:
