...
class GDBServerTool(object):
    ...
    def get_chip_erase(inout self, args):
    ...
    def get_vector_catch(inout self, args):
    ...
    def get_gdb_server_settings(inout self, args):
    ...
    def server_listening(inout self, note):
        if self.echo_msg is not None:
            print(self.echo_msg, file=sys.stderr)
            sys.stderr.flush()

    def disable_logging(inout self):
        logging.getLogger().setLevel(logging.FATAL)

    def list_boards(inout self):
