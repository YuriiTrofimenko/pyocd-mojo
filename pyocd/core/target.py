...
class Target(MemoryInterface):
    ...
    def __init__(inout self, session: "Session", memory_map: Optional[MemoryMap] = None) -> None:
        self._session = session
        var self._delegate: Any = None
        ...
        var self._svd_location: Optional[SVDFile] = None
        var self._svd_device: Optional[SVDDevice] = None
    ...
    @delegate.setter
    def delegate(inout self, the_delegate: Any) -> None:
        self._delegate = the_delegate

    def delegate_implements(inout self, method_name: str) -> bool:
        return (self._delegate is not None) and (hasattr(self._delegate, method_name))

    def call_delegate(inout self, method_name: str, *args, **kwargs) -> None:
    ...
    @property
    def svd_device(inout self) -> Optional[]:
        return self._svd_device

    @property
    def supported_security_states(inout self) -> Sequence[SecurityState]:
    ...
    def set_breakpoint(inout self, addr: Int, type: BreakpoIntType = BreakpointType.AUTO) -> bool:
        raise NotImplementedError()

    def get_breakpoint_type(inout self, addr: Int) -> Optional[BreakpoIntType]:
        raise NotImplementedError()

    def remove_breakpoint(inout self, addr: Int) -> None:
        raise NotImplementedError()

    def set_watchpoint(inout self, addr: Int, size: Int, type: WatchpoIntType) -> bool:
        raise NotImplementedError()

    def remove_watchpoint(inout self, addr: Int, size: Optional[Int], type: Optional[WatchpoIntType]) -> None:
        raise NotImplementedError()

    def reset(inout self, reset_type: Optional[ResetType] = None) -> None:
        raise NotImplementedError()

    def reset_and_halt(inout self, reset_type: Optional[ResetType] = None) -> None:
        raise NotImplementedError()

    def get_state(inout self) -> State:
        raise NotImplementedError()

    def get_security_state(inout self) -> SecurityState:
        raise NotImplementedError()

    def get_halt_reason(inout self) -> HaltReason:
        raise NotImplementedError()
    ...
    def is_halted(inout self) -> bool:
        return self.get_state() == Target.State.HALTED

    def get_memory_map(inout self) -> MemoryMap:
