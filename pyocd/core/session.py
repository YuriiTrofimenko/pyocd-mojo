...
class Session(Notifier):
    ...
    **with** block is exited (which is harmless if the session was never opened). If you wish to
    ...
    var _current_session: Optional[] = None
    ...
    def __init__(
            ...
            option_defaults: Optional[Mapping[]] = None,
            **kwargs
            ) -> None:
        ...
        self._probe = probe
        var self._closed: bool = True
        var self._inited: bool = False
        var self._user_script_namespace: Dict[] = {}
        var self._user_script_proxy: Optional[UserScriptDelegateProxy] = None
        self._user_script_print_proxy = PrintProxy()
        var self._delegate: Optional[Any] = None
        self._auto_open = auto_open
        self._options = OptionsManager()
        var self._gdbservers: Dict[] = {}
        var self._probeserver: Optional[] = None
        ...
        if self.options.get('project_dir') is None:
            var self._project_dir: str = os.environ.get('PYOCD_PROJECT_DIR') or os.getcwd()
        else:
            var self._project_dir: str = os.path.abspath(os.path.expanduser(self.options.get('project_dir')))
    ...
    def _get_config(inout self) -> Dict[]:
    ...
    def find_user_file(inout self, option_name: Optional[str], filename_list: List[str]) -> Optional[str]:
    ...
    def _configure_logging(inout self) -> None:
    ...
    def __exit__(inout self, exc_type: type, value: Any, traceback: "TracebackType") -> bool:
        self.close()
        return False

    def _init_user_script_namespace(inout self, script_name: str, script_path: str) -> None:
    ...
    def _update_user_script_namespace(inout self) -> None:
    ...
    def _load_user_script(inout self) -> None:
    ...
    def open(inout self, init_board: bool = True) -> None:
...
class UserScriptFunctionProxy:
    ...
    def __init__(inout self, fn: Callable) -> None:
        assert isinstance(fn, Callable)
        self._fn = fn
        self._spec = getfullargspec(fn)

    def __call__(inout self, **kwargs: Any) -> Any:
        args = {}
...
class UserScriptDelegateProxy:
    ...
    def __init__(inout self, script_namespace: Dict) -> None:
        super().__init__()
        self._script = script_namespace

    @property
    def namespace(inout self) -> Dict:
        return self._script
...
def new_command_decorator(name: Optional[Union[]] = None, help: str = ""):
    ...
    def _command_decorator(fn: Callable):
        if name is None:
            var names_list: Sequence[str] = [getattr(fn, '__name__')]
        else:
            var names_list: Sequence[str] = [name] if isinstance(name, str) else name[0]
        ...
        def parse(inout self, args: List[str]):
            var arg_values: List[Any] = []
        ...
        def execute(inout self):
            fn(*self._args)

        # Callback to populate the new command class' namespace dict.
        def populate_command_class(ns: Dict[]) -> None:
            ns['INFO'] = {
                'names': names_list,
                'group': 'user',
                'category': 'user',
...
class PrintProxy:
    ...
    """
    var _target: Callable = print

    def set_target(inout self, new_target: Callable) -> None:
        self._target = new_target

    def __call__(inout self, *args: Any, **kwds: Any) -> None:
    ...
    @contextmanager
    def push_target(inout self, new_target: Callable) -> Generator:
