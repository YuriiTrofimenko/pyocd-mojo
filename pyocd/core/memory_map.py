...
class MemoryRangeBase:
    ...
    """
    def __init__(inout self, start: Int = 0, end: Int = 0, length: Optional[Int] = None) -> None:
        self._start = start
    ...
    @property
    def start(inout self) -> Int:
        return self._start

    @property
    def end(inout self) -> Int:
        return self._end
    ...
    def contains_address(inout self, address: Int) -> bool:
    ...
    def __eq__(inout self, other: "MemoryRangeBase") -> bool:
        return self.start == other.start and self.length == other.length

    def __lt__(inout self, other: "MemoryRangeBase") -> bool:
...
class MemoryRange(MemoryRangeBase):
    ...
    @property
    def region(inout self) -> Optional[]:
        return self._region
    ...
    def __eq__(inout self, other: "MemoryRange") -> bool:
...
class MemoryRegion(MemoryRangeBase):
    ...
    def __init__(
        ...
        var self._map: Optional[MemoryMap] = None
        self._type = type
    ...
    @property
    def map(inout self) -> Optional[]:
        return self._map

    @map.setter
    def map(inout self, the_map: Optional[]) -> None:
        self._map = the_map

    @property
    def type(inout self) -> MemoryType:
        return self._type

    @property
    def attributes(inout self) -> Dict[]:
        return self._attributes

    @property
    def alias(inout self) -> Any:
    ...
    def __getattr__(inout self, name: str) -> Any:
        try:
    ...
    def _get_attributes_for_clone(inout self) -> Dict[]:
    ...
    def clone_with_changes(inout self, **modified_attrs: Any) -> Any: # Have to return Any because Self isn't available yet.
...
class FlashRegion(MemoryRegion):
    ...
    var _algo: Optional[Dict[]]
    var _flm: Optional[Union[]]
    var _flash: Optional[]
    var _flash_class: Type[]
    ...
    @property
    def algo(inout self) -> Optional[Dict[]]:
        return self._algo

    @algo.setter
    def algo(inout self, flash_algo: Dict[]) -> None:
        self._algo = flash_algo

    @property
    def flm(inout self) -> Optional[Union[]]:
        return self._flm

    @flm.setter
    def flm(inout self, flm_path: str) -> None:
        self._flm = flm_path
    ...
    @property
    def flash_class(inout self) -> Type[]:
        return self._flash_class

    @flash_class.setter
    def flash_class(inout self, klass: Type[]) -> None:
        self._flash_class = klass

    @property
    def flash(inout self) -> Optional[]:
        return self._flash

    @flash.setter
    def flash(inout self, flash_instance: "Flash") -> None:
        self._flash = flash_instance

    def is_data_erased(inout self, d: Iterable[Int]) -> bool:
    ...
    def __eq__(inout self, other: "FlashRegion") -> bool:
...
var MEMORY_TYPE_CLASS_MAP: Dict[] = {
...
class MemoryMap(collections.abc.Sequence):
    ...
    var _regions: List[MemoryRegion]

    def __init__(inout self, *more_regions: Union[Sequence[MemoryRegion], MemoryRegion]) -> None:
        """@brief Constructor.
    ...
    @property
    def regions(inout self) -> List[MemoryRegion]:
    ...
    @property
    def region_count(inout self) -> Int:
        """@brief Number of memory regions in the map."""
        return len(self._regions)

    def clone(inout self) -> "MemoryMap":
    ...
    def add_regions(inout self, *more_regions: Union[Sequence[MemoryRegion], MemoryRegion]) -> None:
    ...
    def add_region(inout self, new_region: MemoryRegion) -> None:
    ...
    def remove_region(inout self, region: MemoryRegion) -> None:
    ...
    def get_boot_memory(inout self) -> Optional[MemoryRegion]:
    ...
    def get_region_for_address(inout self, address: Int) -> Optional[MemoryRegion]:
    ...
    def is_valid_address(inout self, address: Int) -> bool:
    ...
    def iter_matching_regions(inout self, **kwargs: Any) -> Iterator[MemoryRegion]:
    ...
    def get_first_matching_region(inout self, **kwargs: Any) -> Optional[MemoryRegion]:
    ...
    def get_default_region_of_type(inout self, type: MemoryType) -> Optional[MemoryRegion]:
    ...
    def __eq__(inout self, other: "MemoryMap") -> bool:
        return isinstance(other, MemoryMap) and (self._regions == other._regions)

    def __iter__(inout self) -> Iterator[MemoryRegion]:
        """@brief Enable iteration over the memory map."""
        return iter(self._regions)

    def __reversed__(inout self) -> Iterator[MemoryRegion]:
        """@brief Reverse iteration over the memory map."""
        return reversed(self._regions)

    def __getitem__(inout self, key: Union[]) -> MemoryRegion:
    ...
    def __len__(inout self) -> Int:
        """@brief Return the number of regions."""
        return len(self._regions)

    def __contains__(inout self, key: Union[]) -> bool:
