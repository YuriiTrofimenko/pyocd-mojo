...
class TransferFaultError(TransferError):
    ...
    """
    def __init__(inout self, *args, **kwargs):
    ...
    @property
    def fault_address(inout self):
        return self._address

    @fault_address.setter
    def fault_address(inout self, addr):
        self._address = addr

    @property
    def fault_end_address(inout self):
        return (self._address + self._length - 1) if (self._length is not None) else self._address

    @property
    def fault_length(inout self):
        return self._length

    @fault_length.setter
    def fault_length(inout self, length):
        self._length = length
