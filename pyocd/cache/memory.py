...
class MemoryCache(object):
    ...
    def __init__(inout self, context, core):
        self._context = context
        self._core = core
        self._run_token = -1
        self._reset_cache()

    def _reset_cache(inout self):
    ...
    def _get_ranges(inout self, addr, count):
    ...
    def _read_uncached(inout self, uncached):
    ...
    def _update_metrics(inout self, cached, uncached, addr, size):
        cachedSize = 0
        for iv in cached:
            begin = iv.begin
            end = iv.end
    ...
    def _read(inout self, addr, size):
    ...
    def _merge_data(inout self, combined, addr, size):
    ...
    def _update_contiguous(inout self, cached, addr, value):
        size = len(value)
        end = addr + size
        leadBegin = addr
        leadData = bytearray()
    ...
    def _check_regions(inout self, addr, count):
    ...
    def write_memory_block8(inout self, addr, value):
        if len(value) <= 0:
            return
