...
class TCPClientProbe(DebugProbe):
    ...
    def __init__(inout self, unique_id):
        """@brief Constructor."""
    ...
    @property
    def request_id(inout self):
        """@brief Generate a new request ID."""
        rid = self._request_id
        self._request_id += 1
        return rid

    def _perform_request_without_raise(inout self, request: str, *args: Any) -> Tuple[]:
    ...
    def _perform_request(inout self, request: str, *args: Any) -> Any:
    ...
    def _read_property(inout self, name, default=None):
        if not self.is_open:
            return default
    ...
    def write_dp(inout self, addr, data):
    ...
    def write_ap(inout self, addr, data):
        self._perform_request('write_ap', addr, data)

    def read_ap_multiple(inout self, addr, count=1, now=True):
    ...
    def write_ap_multiple(inout self, addr, values):
        self._perform_request('write_ap_multiple', addr, values)

    def get_memory_interface_for_ap(inout self, ap_address):
...
class RemoteMemoryInterface(MemoryInterface):
    ...
    def __init__(inout self, remote_probe, handle):
        self._remote_probe = remote_probe
        self._handle = handle

    def write_memory(inout self, addr, data, transfer_size=32, **attrs):
        assert transfer_size in (8, 16, 32)
        self._remote_probe._perform_request('write_mem', self._handle, addr, data, transfer_size)

    def read_memory(inout self, addr, transfer_size=32, now=True, **attrs):
        assert transfer_size in (8, 16, 32)
