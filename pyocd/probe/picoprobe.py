...
class PicoLink(object):
    ...
    @classmethod
    def enumerate_picoprobes(cls, uid=None) -> List[]:
    ...
    def q_read_bits(inout self, bits):
    ...
    def q_write_bits(inout self, data, bits=None):
    ...
    # ------------------------------------------- #
    def _next_id(inout self):
        """@brief Returns a progressive id for a Picoprobe command"""
        id = self._id
        self._id = (self._id + 1) % 0x100
        return id

    def _queue_cmd_header(inout self, cmd, bits, length=0, id=None):
...
class Picoprobe(DebugProbe):
    ...
    def __init__(inout self, picolink):
    ...
    @ property
    def supported_wire_protocols(inout self):
    ...
    # ------------------------------------------- #
    def connect(inout self, protocol=None):
    ...
    # ------------------------------------------- #
    def read_dp(inout self, addr, now=True):
    ...
    def write_dp(inout self, addr, value):
        self._write_reg(addr, self.DP, value)

    def read_ap(inout self, addr, now=True):
    ...
    def write_ap(inout self, addr, value):
        self.write_ap_multiple(addr, (value,))

    def _safe_read_ap_multiple(inout self, addr, count=1, now=True):
    ...
    def _safe_write_ap_multiple(inout self, addr, values):
        # Send repeated read request for the AP
        for v in values:
            self._write_reg(addr, self.AP, v)

    def _bulk_read_ap_multiple(inout self, addr, count=1, now=True):
    ...
    def _bulk_write_ap_multiple(inout self, addr, values):
        acks = []
        left = len(values)
        done = 0
    ...
    def _read_reg(inout self, addr, APnDP):
        # This is a safe read
        self._link.start_queue()
    ...
    def _write_reg(inout self, addr, APnDP, value):
    ...
    def _swd_command(inout self, RnW, APnDP, addr):
    ...
    def _change_options(inout self, notification):
