...
class STLinkUSBInterface:
    ...
    @property
    def version_name(inout self):
        return self._info.version_name

    @property
    def max_packet_size(inout self):
        return self._max_packet_size

    def _flush_rx(inout self):
        assert self._ep_in
    ...
    def _read(inout self, size, timeout=1000):
        assert self._ep_in
    ...
    def transfer(inout self, cmd, writeData=None, readSize=None, timeout=1000):
        assert self._ep_out
    ...
    def read_swv(inout self, size, timeout=1000):
        assert self._ep_swv
