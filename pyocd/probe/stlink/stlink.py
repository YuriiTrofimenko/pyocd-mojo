...
class STLink(object):
    ...
    def __init__(inout self, device):
        self._device = device
        self._hw_version = 0
    ...
    def get_board_id(inout self) -> Optional[str]:
    ...
    def _check_version(inout self, min_version):
    ...
    @property
    def serial_number(inout self):
        return self._device.serial_number

    @property
    def hw_version(inout self):
        return self._hw_version

    @property
    def jtag_version(inout self):
        return self._jtag_version

    @property
    def version_str(inout self):
        return self._version_str

    @property
    def target_voltage(inout self):
        return self._target_voltage

    @property
    def supports_banked_dp(inout self):
    ...
    def get_target_voltage(inout self):
    ...
    def set_prescaler(inout self, prescaler: Int) -> None:
    ...
    def set_swd_frequency(inout self, freq: Union[] = 1800000):
        with self._lock:
            if self._hw_version >= 3:
    ...
    def set_jtag_frequency(inout self, freq: Union[] = 1120000):
        with self._lock:
            if self._hw_version >= 3:
    ...
    def get_com_frequencies(inout self, protocol: "STLink.Protocol"):
    ...
    def set_com_frequency(inout self, protocol: "STLink.Protocol", freq: Union[]):
        assert self._hw_version >= 3
    ...
    def enter_debug(inout self, protocol):
        with self._lock:
            self.enter_idle()
    ...
    def _check_status(inout self, response):
    ...
    def _clear_sticky_error(inout self):
        with self._lock:
    ...
    def _get_csw_bytes(inout self, csw: Int) -> Tuple[]:
    ...
    def _read_mem(inout self, addr: Int, size: Int, memcmd: Int, maxrx: Int, apsel: Int, csw: Int) -> List[Int]:
        with self._lock:
            result = []
            while size:
    ...
    def _write_mem(inout self, addr: Int, data: Sequence[Int], memcmd: Int, maxtx: Int, apsel: Int, csw: Int) -> None:
        with self._lock:
            while len(data):
    ...
    def read_mem32(inout self, addr: Int, size: Int, apsel: Int, csw: Int):
    ...
    def write_mem32(inout self, addr: Int, data: Sequence[Int], apsel: Int, csw: Int):
    ...
    def read_mem16(inout self, addr: Int, size: Int, apsel: Int, csw: Int):
    ...
    def write_mem16(inout self, addr: Int, data: Sequence[Int], apsel: Int, csw: Int):
    ...
    def read_dap_register(inout self, port, addr):
    ...
    def write_dap_register(inout self, port, addr, value):
        assert (addr >> 16) == 0, "register address must be 16-bit"
    ...
    def swo_start(inout self, baudrate):
        with self._lock:
            bufferSize = 4096
