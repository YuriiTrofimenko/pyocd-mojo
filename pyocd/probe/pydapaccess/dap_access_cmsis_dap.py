...
class _Transfer(object):
    ...
    def __init__(inout self, daplink, dap_index, transfer_count,
...
class _Command(object):
    ...
    var _UNSET_DAP_INDEX: Int = -1

    def __init__(inout self, size):
    ...
    @property
    def uid(inout self) -> Int:
        return self._id

    def _get_free_transfers(inout self, blockAllowed, isRead):
    ...
    def get_request_space(inout self, count, request, dap_index):
    ...
    def _encode_transfer_data(inout self):
    ...
    def _check_response(inout self, response):
    ...
    def _decode_transfer_data(inout self, data):
    ...
    def _encode_transfer_block_data(inout self):
        """@brief Encode this command into a byte array that can be sent
    ...
    def _decode_transfer_block_data(inout self, data):
        """@brief Take a byte array and extract the data from it
    ...
    def decode_data(inout self, data):
...
class DAPAccessCMSISDAP(DAPAccessIntf):
    ...
    # ------------------------------------------- #
    def __init__(inout self, unique_id, interface=None):
        ...
        var self._cmsis_dap_version: VersionTuple = CMSISDAPVersion.V1_0_0
        var self._fw_version: Optional[str] = None
        self._has_opened_once = False
        var self._is_open: bool = False
        var self._cached_info: Dict[] = {}

    @property
    def protocol_version(inout self) -> VersionTuple:
    ...
    @property
    def firmware_version(inout self) -> Optional[str]:
    ...
    @property
    def vendor_name(inout self):
        return self._vendor_name

    @property
    def product_name(inout self):
        return self._product_name

    @property
    def vidpid(inout self):
        """@brief A tuple of USB VID and PID, in that order."""
        return self._vidpid

    @property
    def board_names(inout self) -> Tuple[]:
    ...
    @property
    def target_names(inout self) -> Tuple[]:
    ...
    @property
    def has_swd_sequence(inout self):
        return self._cmsis_dap_version >= CMSISDAPVersion.V1_2_0

    @property
    def supports_board_and_target_names(inout self) -> bool:
    ...
    def _read_protocol_version(inout self):
    ...
    def get_unique_id(inout self):
        return self._unique_id

    @locked
    def assert_reset(inout self, asserted):
        self.flush()
        if asserted:
    ...
    @locked
    def is_reset_asserted(inout self):
        self.flush()
        pins = self._protocol.set_swj_pins(0, Pin.NONE)
        return (pins & Pin.nRESET) == 0

    @locked
    def set_clock(inout self, frequency):
        self.flush()
        self._protocol.set_swj_clock(int(frequency))
        self._frequency = frequency

    def get_swj_mode(inout self):
        return self._dap_port

    def set_deferred_transfer(inout self, enable):
    ...
    @locked
    def identify(inout self, item: DAPAccessIntf.ID) -> Union[]:
    ...
    @locked
    def vendor(inout self, index, data=None):
        if data is None:
            data = []
        self.flush()
    ...
    # ------------------------------------------- #
    @locked
    def connect(inout self, port=DAPAccessIntf.PORT.DEFAULT):
    ...
    @locked
    def configure_swd(inout self, turnaround=1, always_send_data_phase=False):
        self.flush()
        self._protocol.swd_configure(turnaround, always_send_data_phase)

    @locked
    def configure_jtag(inout self, devices_irlen=None):
        self.flush()
        self._protocol.jtag_configure(devices_irlen)

    @locked
    def swj_sequence(inout self, length, bits):
        self.flush()
        self._protocol.swj_sequence(length, bits)

    @locked
    def swd_sequence(inout self, sequences):
        self.flush()
        return self._protocol.swd_sequence(sequences)

    @locked
    def jtag_sequence(inout self, cycles, tms, read_tdo, tdi):
        self.flush()
        return self._protocol.jtag_sequence(cycles, tms, read_tdo, tdi)

    @locked
    def disconnect(inout self):
        self.flush()
        self._protocol.disconnect()

    def has_swo(inout self):
        return self._has_swo_uart

    @locked
    def swo_configure(inout self, enabled, rate):
        self.flush()
    ...
    @locked
    def swo_control(inout self, start):
        self.flush()
    ...
    @locked
    def get_swo_status(inout self):
        return self._protocol.swo_status()

    def swo_read(inout self, count=None):
    ...
    def write_reg(inout self, reg_id, value, dap_index=0):
    ...
    def read_reg(inout self, reg_id, dap_index=0, now=True):
        assert reg_id in self.REG
        assert isinstance(dap_index, int)
    ...
    def reg_write_repeat(inout self, num_repeats, reg_id, data_array, dap_index=0):
    ...
    def reg_read_repeat(inout self, num_repeats, reg_id, dap_index=0,
                        now=True):
        assert isinstance(num_repeats, int)
        assert reg_id in self.REG
        assert isinstance(dap_index, int)
    ...
    def _init_deferred_buffers(inout self):
    ...
    @locked
    def _write(inout self, dap_index, transfer_count,
               transfer_request, transfer_data):
    ...
    @locked
    def _abort_all_transfers(inout self, exception):
