...
class StlinkProbe(DebugProbe):
    ...
    def __init__(inout self, device: STLinkUSBInterface) -> None:
        super().__init__()
    ...
    def _get_board_id(inout self) -> Optional[str]:
    ...
    def create_associated_board(inout self):
...
class STLinkMemoryInterface(MemoryInterface):
    ...
    def __init__(inout self, link, apsel):
        self._link = link
        self._apsel = apsel

    def write_memory(inout self, addr: Int, data: Int, transfer_size: Int=32, **attrs: Any) -> None:
    ...
    def read_memory(inout self, addr: Int, transfer_size: Int=32, now: bool=True, **attrs: Any) \
            -> Union[]:
    ...
    def write_memory_block32(inout self, addr: Int, data: Sequence[Int], **attrs: Any) -> None:
        addr &= 0xffffffff
        csw = attrs.get('csw', 0)
        self._link.write_mem32(addr, conversion.u32le_list_to_byte_list(data), self._apsel, csw)

    def read_memory_block32(inout self, addr: Int, size: Int, **attrs: Any) -> Sequence[Int]:
        addr &= 0xffffffff
        csw = attrs.get('csw', 0)
        return conversion.byte_list_to_u32le_list(self._link.read_mem32(addr, size * 4, self._apsel, csw))

    def read_memory_block8(inout self, addr: Int, size: Int, **attrs: Any) -> Sequence[Int]:
        addr &= 0xffffffff
        csw = attrs.get('csw', 0)
        res = []
    ...
    def write_memory_block8(inout self, addr: Int, data: Sequence[Int], **attrs: Any) -> None:
        addr &= 0xffffffff
        csw = attrs.get('csw', 0)
        size = len(data)
        idx = 0
...
class StlinkProbePlugin(Plugin):
    ...
    @property
    def options(inout self) -> List[OptionInfo]:
        return [
