...
class DebugProbeServer(threading.Thread):
    ...
    def __init__(
        ...
        self._probe = probe
        var self._did_start: bool = False
        var self._is_running: bool = False
    ...
    @property
    def is_running(inout self) -> bool:
        """@brief Whether the server thread is running."""
        return self._is_running

    @property
    def port(inout self) -> Int:
...
class TCPProbeServer(ThreadingTCPServer):
    ...
    def __init__(inout self, server_address: Tuple[], session: "Session", probe: DebugProbe):
        self._session = session
        self._probe = probe
    ...
    def handle_error(inout self, request, client_address):
...
class DebugProbeRequestHandler(StreamRequestHandler):
    ...
    def setup(inout self):
        ...
        var self._next_ap_memif_handle: Int = 0
        var self._ap_memif_handles: Dict[] = {}

        # Create the request handlers dict here so we can reference bound probe methods.
        var self._REQUEST_HANDLERS: Dict[] = {
    ...
    def _send_error_response(inout self, status=1, message=""):
        response_dict = {
    ...
    def _send_response(inout self, result):
        response_dict = {
                "id": self._current_request_id,
                "status": 0,
            }
    ...
    def _get_exception_status_code(inout self, err):
    ...
    def _request__connect(inout self, protocol_name):
    ...
    def _request__get_memory_interface_for_ap(inout self, ap_address_version, ap_nominal_address):
    ...
    def _request__swo_read(inout self):
        return list(self._probe.swo_read())

    def _request__read_mem(inout self, handle, addr, xfer_size):
    ...
    def _request__write_mem(inout self, handle, addr, value, xfer_size):
        # 'write_mem', handle:int, addr:int, value:int, xfer_size:int
        if handle not in self._ap_memif_handles:
            raise exceptions.Error("invalid handle received from remote memory access")
        self._ap_memif_handles[handle].write_memory(addr, value, xfer_size)

    def _request__read_block32(inout self, handle, addr, word_count):
    ...
    def _request__write_block32(inout self, handle, addr, data):
    ...
    def _request__read_block8(inout self, handle, addr, word_count):
    ...
    def _request__write_block8(inout self, handle, addr, data):
