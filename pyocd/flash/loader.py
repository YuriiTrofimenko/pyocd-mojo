...
class RamBuilder(MemoryBuilder):
    ...
    def __init__(inout self, session: "Session", region: "RamRegion") -> None:
        """@brief Constructor."""
        super().__init__()
        self._session = session
        self._region = region
        var self._chunks: List[DataChunk] = []
    ...
    def program(inout self, progress_cb: Optional[ProgressCallback] = None, **kwargs: Any) -> ProgrammingInfo:
...
class MemoryLoader:
    ...
    _session: "Session"
    _map: "MemoryMap"
    var _progress: Optional[ProgressCallback]
    var _builders: Dict[]
    var _total_data_size: Int
    var _progress_offset: Float32
    var _current_progress_fraction: Float32

    var _chip_erase: Optional[bool]
    var _smart_flash: Optional[bool]
    var _trust_crc: Optional[bool]
    var _keep_unwritten: Optional[bool]
    ...
    def add_data(inout self, address, data):
    ...
    def _log_performance(inout self, perf_list):
    ...
    def _progress_cb(inout self, amount):
