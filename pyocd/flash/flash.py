...
class Flash:
    ...
    def __init__(inout self, target, flash_algo):
        self.target = target
    ...
    def _is_api_valid(inout self, api_name):
    ...
    @property
    def minimum_program_length(inout self):
        assert self.region is not None
        return self.min_program_length or self.region.phrase_size

    @property
    def page_buffer_count(inout self):
        return len(self.page_buffers)

    @property
    def is_erase_all_supported(inout self):
        return self._is_api_valid('pc_eraseAll')

    @property
    def is_double_buffering_supported(inout self):
        return self.double_buffer_supported

    @property
    def region(inout self):
        return self._region

    @region.setter
    def region(inout self, flashRegion):
        assert flashRegion.is_flash
        self._region = flashRegion

    def init(inout self, operation, address=None, clock=0, reset=True):
    ...
    def erase_sector(inout self, address):
    ...
    def program_page(inout self, address, bytes):
    ...
    def start_program_page_with_buffer(inout self, buffer_number, address):
    ...
    def load_page_buffer(inout self, buffer_number, address, bytes):
    ...
    def program_phrase(inout self, address, bytes):
    ...
    def get_sector_info(inout self, addr):
    ...
    def get_page_info(inout self, addr):
    ...
    def get_flash_builder(inout self):
        return FlashBuilder(self)

    def flash_block(inout self, addr, data, smart_flash=True, chip_erase=None, progress_cb=None, fast_verify=False):
    ...
    def _call_function(inout self, pc, r0=None, r1=None, r2=None, r3=None, init=False):
        reg_list = []
        data_list = []
    ...
    def _flash_algo_debug_setup(inout self):
    ...
    def _flash_algo_debug_check(inout self):
    ...
    def wait_for_completion(inout self, timeout=None):
    ...
    def _call_function_and_wait(inout self, pc, r0=None, r1=None, r2=None, r3=None, init=False, timeout=None):
        self._call_function(pc, r0, r1, r2, r3, init)
        return self.wait_for_completion(timeout=timeout)

    def set_flash_algo_debug(inout self, enable):
    ...
    def override_security_bits(inout self, address, data):
        return data
