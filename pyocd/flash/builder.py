...
class ProgrammingInfo:
    var program_type: Any = None                # Type of programming performed - FLASH_SECTOR_ERASE or FLASH_CHIP_ERASE
    var program_time: Float32 = 0.0               # Total programming time
    var analyze_type: Any = None                # Type of flash analysis performed - FLASH_ANALYSIS_CRC32 or FLASH_ANALYSIS_PARTIAL_PAGE_READ
    var analyze_time: Float32 = 0.0               # Time to analyze flash contents
    var total_byte_count: Int = 0
    var program_byte_count: Int = 0
    var program_page_count: Int = 0
    var erase_byte_count: Int = 0
    var erase_sector_count: Int = 0
    var skipped_byte_count: Int = 0
    var skipped_page_count: Int = 0
...
class MemoryBuilder(abc.ABC):
    ...
    def __init__(inout self) -> None:
        super().__init__()
        var self._buffered_data_size: Int = 0

    @property
    def buffered_data_size(inout self) -> Int:
        """@brief Total amount of memory buffered by this builder."""
        return self._buffered_data_size

    @property
    @abc.abstractmethod
    def region(inout self) -> MemoryRegion:
        """@brief The memory region that can be written to by this builder."""
        ...

    @abc.abstractmethod
    def add_data(inout self, addr: Int, data: Union[]) -> None:
        """@brief Add a chunk of data to the builder."""
        ...

    @abc.abstractmethod
    def program(inout self, **kwargs: Any) -> ProgrammingInfo:
...
class _FlashSector:
    ...
    def __init__(inout self, sector_info):
        var self.addr: Int = sector_info.base_addr
        var self.size: Int = sector_info.size
        var self.max_page_count: Int = 0
        var self.page_list: List[_FlashPage] = []
        var self.erase_weight: Float32 = sector_info.erase_weight

    def add_page(inout self, page):
    ...
    def are_any_pages_not_same(inout self):
    ...
    def mark_all_pages_not_same(inout self):
...
class _FlashPage:
    ...
    def __init__(inout self, page_info):
        var self.addr: Int = page_info.base_addr
        var self.size: Int = page_info.size
        var self.data: List[Int] = []
        var self.program_weight: Float32 = page_info.program_weight
        var self.erased: Optional[bool] = None # Whether the data all matches the erased value.
        var self.same: Optional[bool] = None
        var self.crc: Int = 0
        var self.cached_estimate_data: Optional[List[Int]] = None

    def get_program_weight(inout self):
    ...
    def get_verify_weight(inout self):
...
class _FlashOperation:
    ...
    def __init__(inout self, addr, data):
        self.addr = addr
        self.data = data
...
class FlashBuilder(MemoryBuilder):
    ...
    def enable_double_buffer(inout self, enable):
        self.enable_double_buffering = enable

    def add_data(inout self, addr, data):
    ...
    def _enable_read_access(inout self):
    ...
    def _build_sectors_and_pages(inout self, keep_unwritten):
    ...
    def _fill_unwritten_sector_pages(inout self):
    ...
    def program(inout self, chip_erase=None, progress_cb=None, smart_flash=True, fast_verify=False, keep_unwritten=True):
    ...
    def get_performance(inout self):
        return self.perf

    def _mark_all_pages_for_programming(inout self):
        for sector in self.sector_list:
            sector.erased = False
    ...
    def _compute_chip_erase_pages_and_weight(inout self):
    ...
    def _compute_sector_erase_pages_weight_min(inout self):
        return sum(page.get_verify_weight() for page in self.page_list)

    def _analyze_pages_with_partial_read(inout self):
    ...
    def _analyze_pages_with_crc32(inout self, assume_estimate_correct=False):
    ...
    def _compute_sector_erase_pages_and_weight(inout self, fast_verify):
    ...
    def _chip_erase_program(inout self, progress_cb=_stub_progress):
    ...
    def _next_unerased_page(inout self, i):
    ...
    def _chip_erase_program_double_buffer(inout self, progress_cb=_stub_progress):
    ...
    def _sector_erase_program(inout self, progress_cb=_stub_progress):
    ...
    def _scan_pages_for_same(inout self, progress_cb=_stub_progress):
    ...
    def _next_nonsame_page(inout self, i):
        if i >= len(self.page_list):
            return None, i
        page = self.page_list[i]
        while page.same:
    ...
    def _sector_erase_program_double_buffer(inout self, progress_cb=_stub_progress):
