...
# Copyright (c) 2020 Fraunhofer-Gesellschaft zur FГ¶rderung der angewandten Forschung e.V.
...
class _DeviceInfo:
    ...
    def __init__(inout self, element: Element, **kwargs):
        var self.element: Element = element
        var self.families: List[str] = kwargs.get('families', [])
        var self.memories: List[Element] = kwargs.get('memories', [])
        var self.algos: List[Element] = kwargs.get('algos', [])
        var self.debugs: List[Element] = kwargs.get('debugs', [])
...
class CmsisPack:
    ...
    """
    def __init__(inout self, file_or_path: Union[]) -> None:
        """@brief Constructor.
...
class CmsisPackDescription:
    ...
    def __init__(inout self, pack: CmsisPack, pdsc_file: IO) -> None:
        """@brief Constructor.
        ...
        var self._state_stack: List[_DeviceInfo] = []
        var self._devices: List[] = []
    ...
    def _parse_devices(inout self, parent: Element) -> None:
    ...
    def _extract_families(inout self) -> List[str]:
    ...
    def _extract_items(inout self, state_info_name: str, filter: Callable[]) -> List[V]:
    ...
    def _extract_memories(inout self) -> List[Element]:
        ...
        """
        def get_start_and_size(elem: Element) -> Tuple[]:
            try:
    ...
    def _extract_algos(inout self) -> List[Element]:
    ...
    def _extract_debugs(inout self) -> List[Element]:
...
class CmsisPackDevice:
    ...
    def __init__(inout self, pack: CmsisPack, device_info: _DeviceInfo):
        """@brief Constructor.
        @param self
        ...
        """
        var self._pack: CmsisPack = pack
        var self._info: _DeviceInfo = device_info
        var self._part: str = _get_part_number_from_element(device_info.element)
        var self._regions: List[MemoryRegion] = []
        var self._saw_startup: bool = False
        var self._default_ram: Optional[MemoryRegion] = None
        var self._memory_map: Optional[MemoryMap] = None

    def _build_memory_regions(inout self) -> None:
    ...
    def _get_containing_region(inout self, addr: Int) -> Optional[MemoryRegion]:
    ...
    def _build_flash_regions(inout self) -> None:
    ...
    def _split_flash_region_by_sector_size(inout self,
            region: MemoryRegion,
            page_size: Int,
            algo: Dict[],
    ...
    def _find_matching_algo(inout self, region: MemoryRegion) -> Element:
    ...
    def _load_flash_algo(inout self, filename: str) -> Optional[PackFlashAlgo]:
    ...
    @property
    def part_number(inout self) -> str:
    ...
    @property
    def svd(inout self) -> Optional[IO[bytes]]:
