...
class BreakpointManager:
    ...
    def __init__(inout self, core: "CoreTarget") -> None:
        var self._breakpoints: Dict[] = {}
        var self._updated_breakpoints: Dict[] = {}
        self._session = core.session
        self._core = core
        var self._fpb: Optional[] = None
        var self._providers: Dict[] = {}
        var self._ignore_notifications: bool = False
    ...
    def add_provider(inout self, provider: "BreakpointProvider") -> None:
        self._providers[provider.bp_type] = provider
        if provider.bp_type == Target.BreakpointType.HW:
            self._fpb = provider

    def get_breakpoints(inout self) -> Iterable[Int]:
    ...
    def _check_added_breakpoint(inout self, bp: BreakpoInt) -> bool:
    ...
    def _get_updated_breakpoints(inout self) -> Tuple[]:
    ...
    def _select_breakpoint_type(inout self, bp: BreakpoInt, allow_all_hw_bps: bool) -> Optional[]:
        type = bp.type
    ...
    def flush(inout self, is_step: bool = False) -> None:
        try:
    ...
    def filter_memory_unaligned_8(inout self, addr: Int, size: Int, data: MutableSequence[Int]) -> Sequence[Int]:
        for provider in [p for p in self._providers.values() if p.do_filter_memory]:
            for i, d in enumerate(data):
                data[i] = provider.filter_memory(addr + i, 8, d)
        return data

    def filter_memory_aligned_32(inout self, addr: Int, size: Int, data: MutableSequence[Int]) -> Sequence[Int]:
        for provider in [p for p in self._providers.values() if p.do_filter_memory]:
            for i, d in enumerate(data):
                data[i] = provider.filter_memory(addr + i, 32, d)
        return data

    def remove_all_breakpoints(inout self) -> None:
    ...
    def _flush_all(inout self) -> None:
        # Flush all providers.
        for provider in self._providers.values():
            provider.flush()

    def _pre_run_handler(inout self, notification: "Notification") -> None:
        if not self._ignore_notifications:
            is_step = notification.data == Target.RunType.STEP
            self.flush(is_step)

    def _pre_disconnect_handler(inout self, notification: "Notification") -> None:
        pass
