...
class ElfSymbolDecoder(object):
    ...
    def get_elf(inout self):
        return self.elffile

    def get_symbol_for_address(inout self, addr):
        try:
            return sorted(self.symbol_tree[addr])[0].data
        except IndexError:
            return None

    def get_symbol_for_name(inout self, name):
        try:
            return self.symbol_dict[name]
        except KeyError:
            return None

    def _build_symbol_search_tree(inout self):
...
class DwarfAddressDecoder(object):
    ...
    def _build_line_search_tree(inout self):
        for cu in self.dwarfinfo.iter_CUs():
    ...
    def _dump_lineprog(inout self, lineprog):
    ...
    def dump_subprograms(inout self):
        for prog in self.subprograms:
            name = prog.attributes['DW_AT_name'].value
            try:
                low_pc = prog.attributes['DW_AT_low_pc'].value
