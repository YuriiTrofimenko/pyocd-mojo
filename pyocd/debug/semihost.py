...
class SemihostIOHandler(object):
    ...
    def cleanup(inout self):
        pass

    @property
    def errno(inout self):
        return self._errno

    def _std_open(inout self, fnptr, fnlen, mode):
    ...
    def open(inout self, fnptr, fnlen, mode):
        raise NotImplementedError()

    def close(inout self, fd):
        raise NotImplementedError()
    ...
    def istty(inout self, fd):
        raise NotImplementedError()

    def seek(inout self, fd, pos):
        raise NotImplementedError()

    def flen(inout self, fd):
        raise NotImplementedError()

    def remove(inout self, ptr, length):
        raise NotImplementedError()

    def rename(inout self, oldptr, oldlength, newptr, newlength):
        raise NotImplementedError()
...
class InternalSemihostIOHandler(SemihostIOHandler):
    ...
    def _is_valid_fd(inout self, fd):
...
class ConsoleIOHandler(SemihostIOHandler):
    ...
    def __init__(inout self, stdin_file, stdout_file=None):
...
class SemihostAgent(object):
    ...
    def __init__(inout self, context, io_handler=None, console=None):
    ...
    def check_and_handle_semihost_request(inout self):
    ...
    def _get_args(inout self, args, count):
    ...
    def _get_string(inout self, ptr, length=None):
    ...
    def handle_sys_open(inout self, args):
    ...
    def handle_sys_read(inout self, args):
    ...
    def handle_sys_readc(inout self, args):
        TRACE.debug("Semihost: readc")
        return self.console.readc()

    def handle_sys_iserror(inout self, args):
        raise NotImplementedError()

    def handle_sys_istty(inout self, args):
        fd = self._get_args(args, 1)
        TRACE.debug("Semihost: istty fd=%d", fd)
        return self.io_handler.istty(fd)

    def handle_sys_seek(inout self, args):
        fd, pos = self._get_args(args, 2)
        TRACE.debug("Semihost: seek fd=%d pos=%d", fd, pos)
        return self.io_handler.seek(fd, pos)

    def handle_sys_flen(inout self, args):
        fd = self._get_args(args, 1)
        TRACE.debug("Semihost: flen fd=%d", fd)
        return self.io_handler.flen(fd)

    def handle_sys_tmpnam(inout self, args):
        raise NotImplementedError()

    def handle_sys_remove(inout self, args):
        ptr, length = self._get_args(args, 2)
        return self.io_handler.remove(ptr, length)

    def handle_sys_rename(inout self, args):
        oldptr, oldlength, newptr, newlength = self._get_args(args, 4)
        return self.io_handler.rename(oldptr, oldlength, newptr, newlength)

    def handle_sys_clock(inout self, args):
        now = time.time()
        delta = now - self.start_time
        return int(delta * 100)

    def handle_sys_time(inout self, args):
        now = datetime.datetime.now()
        delta = now - self.EPOCH
        seconds = (delta.days * 86400) + delta.seconds
        return seconds

    def handle_sys_system(inout self, args):
        raise NotImplementedError()

    def handle_sys_errno(inout self, args):
        return self.io_handler.errno

    def handle_sys_get_cmdline(inout self, args):
        raise NotImplementedError()

    def handle_sys_heapinfo(inout self, args):
        raise NotImplementedError()

    def handle_sys_exit(inout self, args):
        raise NotImplementedError()

    def handle_sys_elapsed(inout self, args):
        raise NotImplementedError()

    def handle_sys_tickfreq(inout self, args):
        raise NotImplementedError()
