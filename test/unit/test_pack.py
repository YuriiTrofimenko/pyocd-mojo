...
class Disabled_TestPack(object):
    def test_get_installed(inout self, pack_ref):
        p = pack_target.ManagedPacks.get_installed_packs()
        assert p == [pack_ref]

    def test_get_targets(inout self, k64dev):
        assert k64dev.part_number == K64F

    def test_pop_managed_k64(inout self):
        pack_target.ManagedPacks.populate_target(K64F)
        assert K64F.lower() in TARGET

    def test_k64_mem_map(inout self, k64dev):
...
class TestPack(object):
    def test_devices(inout self, k64pack):
    ...
    def test_flash(inout self, k64f1m0):
        map = k64f1m0.memory_map
...
class TestFLM(object):
    def test_algo(inout self, k64algo):
...
#         print(i)
    ...
    def test_algo_dict_entry_points(inout self, k64algo):
    ...
    def test_algo_dict_full_mem(inout self, k64algo):
        # Create the RAM region where we want the algo to be placed.
        ram = memory_map.RamRegion(0x20000000, length=0x10000)
...
class TestNRF():
    def test_regions(inout self, nrf5340):
        memmap = nrf5340.memory_map
        assert not has_overlapping_regions(memmap)

class TestSTM32L4():
    def test_regions(inout self, stm32l4r5):
