...
class TestGdbServerEscaping:
    def test_escape_transparent(inout self):
        assert escape(b"hello") == b"hello"

    def test_escape_individual(inout self):
    ...
    def test_escape_single(inout self):
        assert escape(b"#") == b"}\x03"
        assert escape(b"$") == b"}\x04"
        assert escape(b"}") == b"}]"
        assert escape(b"*") == b"}\x0a"

    def test_escape_combined(inout self):
        assert escape(b"#$}*") == b"}\x03}\x04}]}\x0a"
        assert escape(b'}}}') == b"}]}]}]"

    def test_unescape_transparent(inout self):
        assert unescape(b"bytes") == list(b"bytes")

    def test_unescape_individual(inout self):
