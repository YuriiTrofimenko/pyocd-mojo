...
class TestConversionUtilities(object):
    ...
    def test_byte_list_to_nbit_le_list_empty(inout self, w):
    ...
    def test_nbit_le_list_to_byte_list(inout self, w):
    ...
    def test_byte_list_to_nbit_le_list(inout self, w, r):
    ...
    def test_byte_list_to_nbit_le_list_nz_pad(inout self, w, r):
    ...
    def test_nbit_le_list_to_byte_list(inout self, w, i):
    ...
    def test_nbit_le_list_to_byte_list_lg(inout self, w, i, r):
        assert nbit_le_list_to_byte_list(i, w) == r

    def test_byte_list_to_u32le_list_empty(inout self):
        assert byte_list_to_u32le_list([]) == []

    def test_byteListToU32leList(inout self):
        data = range(32)
    ...
    def test_byte_list_to_u32le_list_unaligned(inout self):
    ...
    def test_u16leListToByteList(inout self):
        data = [0x3412, 0xFEAB]
    ...
    def test_byteListToU16leList(inout self):
    ...
    def test_align_up(inout self, args, result):
    ...
    def test_align_down(inout self, args, result):
...
class TestGdbEscape(object):
    ...
    def test_escape_passthrough(inout self, data):
    ...
    def test_escape_1(inout self, data, expected):
        assert escape(data) == expected

    def test_escape_2(inout self):
        assert escape(b'1234#09*xyz') == b'1234}\x0309}\x0axyz'

    # Verify all chars that shouldn't be escaped pass through unmodified.
    @pytest.mark.parametrize("data",
        [six.int2byte(x) for x in range(256) if (x not in ESCAPEES)])
    def test_unescape_passthrough(inout self, data):
