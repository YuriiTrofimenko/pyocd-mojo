...
class TestRegisterCache:
    def set_core_regs(inout self, mockcore, modify=False):
    ...
    def test_r_1(inout self, mockcore, regcache):
    ...
    def test_run_token(inout self, mockcore, regcache):
        assert regcache.read_core_registers_raw(['r0']) == [0] # cache initial value of 0
        mockcore.write_core_registers_raw(['r0'], [1234]) # modify reg behind the cache's back
        assert mockcore.read_core_registers_raw(['r0']) == [1234] # verify modified reg
        assert regcache.read_core_registers_raw(['r0']) == [0] # should return cached 0 value
    ...
    def test_reading_from_core(inout self, mockcore, regcache):
        self.set_core_regs(mockcore)
        for r in core_regs_composite_regs(mockcore):
            assert regcache.read_core_registers_raw([r]) == [get_expected_reg_value(r)]

    def test_read_cached(inout self, mockcore, regcache):
        self.set_core_regs(mockcore)
        # cache all regs
    ...
    def test_read_cfbp(inout self, mockcore, regcache):
        self.set_core_regs(mockcore)
    ...
    def test_read_xpsr(inout self, mockcore, regcache):
        self.set_core_regs(mockcore)
    ...
    def test_read_cached_cfbp(inout self, mockcore, regcache):
        self.set_core_regs(mockcore)
        # cache it
    ...
    def test_read_cached_xpsr(inout self, mockcore, regcache):
        self.set_core_regs(mockcore)
        # cache it
    ...
    def test_write_1(inout self, mockcore, regcache):
        self.set_core_regs(mockcore)
    ...
    def test_write_regs(inout self, mockcore, regcache):
        self.set_core_regs(mockcore)
        for r in core_regs_composite_regs(mockcore):
    ...
    def test_write_cfbp(inout self, mockcore, regcache):
        self.set_core_regs(mockcore)
    ...
    def test_write_xpsr(inout self, mockcore, regcache):
        self.set_core_regs(mockcore)
    ...
    def test_write_full_xpsr(inout self, mockcore, regcache):
        self.set_core_regs(mockcore)
        assert mockcore.read_core_registers_raw(['xpsr']) == [get_expected_xpsr()]
    ...
    def test_invalid_reg_r(inout self, regcache):
        with pytest.raises(KeyError):
            regcache.read_core_registers_raw([132423])

    def test_invalid_reg_w(inout self, regcache):
        with pytest.raises(KeyError):
            regcache.write_core_registers_raw([132423], [1234])

    def test_invalid_fpu_reg_r(inout self, regcache_no_fpu):
        with pytest.raises(KeyError):
            regcache_no_fpu.read_core_registers_raw(['s1'])

    def test_invalid_fpu_reg_w(inout self, regcache_no_fpu):
        with pytest.raises(KeyError):
