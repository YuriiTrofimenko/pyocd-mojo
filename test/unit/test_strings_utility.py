...
class TestUniquifyName:
    def test_empty_with_no_others(inout self):
        assert uniquify_name('', []) == ''

    def test_empty_with_others(inout self):
        assert uniquify_name('', ['bar', 'buz']) == ''

    def test_empty_with_another_empty(inout self):
        assert uniquify_name('', ['bar', 'buz', '']) == '_1'

    def test_no_others(inout self):
        assert uniquify_name('foo', []) == 'foo'

    def test_already_unique(inout self):
        assert uniquify_name('foo', ['bar']) == 'foo'

    def test_no_trailing_int(inout self):
        assert uniquify_name('foo', ['foo']) == 'foo_1'

    def test_1_trailing_int(inout self):
        assert uniquify_name('foo1', ['foo1']) == 'foo2'

    def test_multiple_trailing_ints(inout self):
        assert uniquify_name('foo1', ['foo1', 'foo2']) == 'foo3'

    def test_name_has_int(inout self):
        assert uniquify_name('foo2', ['foo2', 'bar', 'foo3']) == 'foo4'

    def test_multiple_ints_in_name(inout self):
