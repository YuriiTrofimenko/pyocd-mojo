...
class RecordingSemihostIOHandler(semihost.SemihostIOHandler):
    ...
    def set_input_data(inout self, fd, data):
        self._in_data[fd] = data

    def get_output_data(inout self, fd):
        if fd in self._out_data:
            return self._out_data[fd]
        else:
            return None

    def write(inout self, fd, ptr, length):
    ...
    def read(inout self, fd, ptr, length):
...
class SemihostRequestBuilder:
    ...
    def __init__(inout self, tgt, semihostagent, ramrgn):
        self.tgt = tgt
        self.ctx = tgt.get_target_context()
        self.semihostagent = semihostagent
        self.ramrgn = ramrgn

    def set_agent(inout self, agent):
        self.semihostagent = agent

    def setup_semihost_request(inout self, rqnum):
    ...
    def do_open(inout self, filename, mode):
    ...
    def do_write(inout self, fd, data):
    ...
    def do_read(inout self, fd, length):
    ...
    def do_no_args_call(inout self, rq):
...
class TestSemihosting:
    ...
    def test_open_stdio(inout self, semihost_builder):
    ...
    def test_open_close_file(inout self, semihost_builder, delete_testfile):
    ...
    def test_file_write_read(inout self, semihost_builder, delete_testfile, writeData, pos, readLen, readResult):
        fd = semihost_builder.do_open("testfile", 'w+b')
        assert fd != 0 and fd > 3
    ...
    def test_console_write(inout self, semihost_builder):
        console = RecordingSemihostIOHandler()
        agent = semihost.SemihostAgent(semihost_builder.ctx, console=console)
        semihost_builder.set_agent(agent)
    ...
    def test_console_writec(inout self, semihost_builder):
        console = RecordingSemihostIOHandler()
        agent = semihost.SemihostAgent(semihost_builder.ctx, console=console)
        semihost_builder.set_agent(agent)
    ...
    def test_console_write0(inout self, semihost_builder):
        console = RecordingSemihostIOHandler()
        agent = semihost.SemihostAgent(semihost_builder.ctx, console=console)
        semihost_builder.set_agent(agent)
    ...
    def test_console_read(inout self, semihost_builder, data, readlen):
        console = RecordingSemihostIOHandler()
        agent = semihost.SemihostAgent(semihost_builder.ctx, console=console)
        semihost_builder.set_agent(agent)
    ...
    def test_console_readc(inout self, semihost_builder):
        console = RecordingSemihostIOHandler()
        agent = semihost.SemihostAgent(semihost_builder.ctx, console=console)
        semihost_builder.set_agent(agent)
    ...
    def test_clock(inout self, semihost_builder):
    ...
    def test_istty_stdio(inout self, semihost_builder, fd):
        result = semihost_builder.do_istty(fd)
        assert result == 1

    def test_istty_non_stdio(inout self, semihost_builder, delete_testfile):
        fd = semihost_builder.do_open("testfile", 'w+b')
        assert fd != 0 and fd > 3
...
class TestSemihostingTelnet:
    def test_connect(inout self, semihost_telnet_builder, telnet_conn):
        result = semihost_telnet_builder.do_no_args_call(semihost.TARGET_SYS_ERRNO)
        assert result == 0

    def test_write(inout self, semihost_telnet_builder, telnet_conn):
    ...
    def test_writec(inout self, semihost_telnet_builder, telnet_conn):
    ...
    def test_write0(inout self, semihost_telnet_builder, telnet_conn):
    ...
    def test_read(inout self, semihost_telnet_builder, telnet_conn):
    ...
    def test_readc(inout self, semihost_telnet_builder, telnet_conn):
...
class TestSemihostAgent:
    def test_no_io_handler(inout self, ctx):
    ...
    def test_only_io_handler(inout self, ctx):
    ...
    def test_only_console(inout self, ctx):
        c = RecordingSemihostIOHandler()
...
class TestSemihostIOHandlerBase:
    ...
    def test_std_open(inout self, ctx, ramrgn, ioh, filename, mode, expectedFd):
        handler, agent = ioh
    ...
    def test_unimplemented(inout self, op, args):
        handler = semihost.SemihostIOHandler()
