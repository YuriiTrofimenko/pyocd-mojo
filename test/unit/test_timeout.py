...
class TestTimeout:
    def test_no_timeout(inout self, mock_time, mock_sleep):
    ...
    def test_timeout_a(inout self, mock_time, mock_sleep):
        with Timeout(0.05) as to:
            while to.check():
                mock_sleep(0.01)
        assert to.did_time_out

    def test_timeout_b(inout self, mock_time, mock_sleep):
        timedout = False
        print(repr(time))
        with Timeout(0.05) as to:
            cnt = 0
    ...
    def test_timeout_c(inout self, mock_time, mock_sleep):
        timedout = False
        with Timeout(0.05) as to:
            cnt = 0
            while cnt < 10:
    ...
    def test_timeout_reset(inout self, mock_time, mock_sleep):
        cnt = 0
        cnta = 0
        with Timeout(0.05) as to:
            cnta = 0
