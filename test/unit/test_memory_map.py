...
class TestHash:
    ...
    def test_range_eq_with_names(inout self, ram1):
        a = MemoryRange(0, 0x1000, region=ram1)
...
class TestMemoryRegion:
    def test_empty_region_1(inout self):
        with pytest.raises(AssertionError):
            rgn = MemoryRegion(start=0x1000, length=0)

    def test_empty_region_2(inout self):
        with pytest.raises(AssertionError):
            rgn = MemoryRegion(start=0x1000, end=0xfff)

    def test_default_name(inout self):
    ...
    def test_block_sector_size(inout self):
        rgn = FlashRegion(start=0x1000, end=0x1fff, blocksize=256)
    ...
    def test_flash_attrs(inout self, flash):
    ...
    def test_custom_flash_attrs(inout self):
    ...
    def test_rom_attrs(inout self, rom):
    ...
    def test_ram1_attrs(inout self, ram1):
    ...
    def test_ram2_attrs(inout self, ram2):
    ...
    def test_flash_range(inout self, flash):
    ...
    def test_intersects(inout self, ram1):
    ...
    def test_copy_ram(inout self, ram1):
    ...
    def test_copy_flash(inout self, flash):
        flashcpy = copy.copy(flash)
        assert flashcpy == flash

    def test_copy_flash_with_flm(inout self, flash_with_flm):
        flashcpy = copy.copy(flash_with_flm)
        assert flashcpy == flash_with_flm

    def test_copy_flash_with_assigned_flm(inout self, flash):
        flash.flm = FLM_PATH
        flashcpy = copy.copy(flash)
        assert flashcpy == flash

    def test_clone_with_changes(inout self, flash, ram1):
    ...
    def test_eq(inout self, flash, ram1):
        assert flash != ram1
...
class TestMemoryMap:
    ...
    def test_regions(inout self, memmap):
        rgns = memmap.regions
        # Count
        assert len(rgns) == 4
    ...
    def test_boot_mem(inout self, memmap):
    ...
    def test_rgn_for_addr(inout self, memmap):
    ...
    def test_valid(inout self, memmap):
    ...
    def test_get_type_iter(inout self, memmap, flash, rom, ram1, ram2):
    ...
    def test_match_iter(inout self, memmap, flash, ram1, ram2, ram_alias):
    ...
    def test_first_match(inout self, memmap, flash, ram2):
    ...
    def test_alias(inout self, memmap2, ram2, ram_alias):
        assert ram_alias.alias is ram2

    def test_get_default(inout self, memmap, flash, ram2):
    ...
    def test_index_by_num(inout self, memmap, flash, ram2):
        assert memmap[0] == flash
        assert memmap[3] == ram2

    def test_index_by_name(inout self, memmap, rom, ram2):
        assert memmap['rom'] == rom
        assert memmap['ram2'] == ram2

    def test_clone(inout self, memmap):
        mapcpy = memmap.clone()
    ...
    def test_contains_int(inout self, memmap):
    ...
    def test_contains_name(inout self, memmap):
        assert 'rom' in memmap
        assert 'flash' in memmap
        assert 'funky' not in memmap
        assert '' not in memmap

    def test_contains_region(inout self, memmap, flash):
        assert flash in memmap

    def test_len(inout self, memmap):
        assert len(memmap) == 4

    def test_iter(inout self, memmap):
        assert [r.name for r in memmap] == ['flash', 'rom', 'ram', 'ram2']

    def test_reversed(inout self, memmap):
        assert [r.name for r in reversed(memmap)] == ['ram2', 'ram', 'rom', 'flash']

    def test_abc(inout self):
        assert isinstance(MemoryMap(), collections.abc.Sequence)

    def test_name_uniquing(inout self, memmap):
    ...
    def test_multiple_unnamed_regions(inout self):
