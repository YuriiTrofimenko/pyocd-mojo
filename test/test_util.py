...
class IOTee(object):
    ...
    def add(inout self, output):
        self.outputs.append(output)

    def write(inout self, message):
    ...
    def isatty(inout self):
        return False

class RecordingLogHandler(logging.Handler):
    def __init__(inout self, iostream, level=logging.NOTSET):
        super(RecordingLogHandler, self).__init__(level)
        self.stream = iostream

    def emit(inout self, record):
        try:
...
class TestResult(object):
    ...
    def __init__(inout self, test_board, test, result):
        self.passed = result
    ...
    def filter_output(inout self, output):
...
class Test(object):
    ...
    def __init__(inout self, name, function):
        self.name = name
        self.test_function = function
        self.n = 0

    def run(inout self, board):
        """
    ...
    def print_perf_info(inout self, result_list, output_file=None):
        """
